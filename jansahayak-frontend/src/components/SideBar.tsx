...
import { LuArrowRight, LuChevronRight, LuEllipsisVertical, LuSettings, LuStar, LuUser } from "react-icons/lu";
...
const SideBar = ({ sideBarRef, setCurrentChat, currentChat, chats, setChats }: SideBarProps) => {
    ...
    return (
        <aside
            className={`sidebar sidebar-open z-999 fixed w-68 left-0 px-5 py-10 md:h-screen md:py-5 flex flex-col justify-start bg-ui-background overflow-x-clip border-r border-ui-tertiary/20 shadow-2xl shadow-black/50`}
            ref={sideBarRef}
        >
            {/* Logo and Name */}
            ...
            {
                user &&
                <Link href="#" className="flex flex-row mb-8 md:mb-0 md:mt-4 gap-2 items-center border border-ui-tertiary/20 shadow-sm shadow-black/20 py-2 px-2.5 w-fit rounded-full justify-center hover:bg-ui-tertiary/30 transition-colors duration-100 cursor-pointer">
                    <span className="border border-ui-tertiary/20 bg-ui-tertiary/40 shadow-sm shadow-black/20 rounded-full p-2.5">
                        <LuUser size={16} className="text-texts-dark" />
                    </span>
                    <span
